...
logging.basicConfig(
    filename='bot.log',  # имя файла для сохранения логов
...
BIRTHDAY_MESSAGE = (
    ...
    "Янги ёшингизда барча орзу-ниятларингиз рўёбга чиқсин ва сизга бериладиган имкониятлар чексиз бўлсин!\н\n"
    "Сизга чин дилдан бахт, омад, ривожланиш ва энг муҳими, ўша асл инсоний қадриятларни сақлаб қолишингизни тилайман. "
    "Қарши олинган ҳар бир янги кун сизнинг энг яхши кунларингиздан бўлсин!\н\n"
    "Туғилган кунингиз яна бир бор муборак бўлсин! 🎂✨\н"
...
def send_birthday_greeting(name):
    personalized_message = f"Хурматли {name},\н\n{BIRTHDAY_MESSAGE}"
    bot.send_message(chat_id="1946744681", text=personalized_message)
    logging.info(f"Отправлено поздравление для {name}")
...
def handle_gpt_query(message):
    ...
    logging.info(f"Обращение от {message.from_user.username} ({message.from_user.id}): {message.text}")
